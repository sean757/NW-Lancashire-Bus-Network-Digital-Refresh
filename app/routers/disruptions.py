...
async def get_live_vehicles():
    """Return live bus positions from the in-memory vehicle cache.

    The cache is populated by a background task that polls the SCC University
    SIRI feed every 30 seconds.  Serving cached data means that no matter how
    many concurrent users call this endpoint, only one set of upstream requests
    is made per interval rather than one set per user per interval.
    ...
    """
    return {'vehicles': vehicle_cache.vehicles}